...
function colorizeLevel(useOnlyCustomProps) {
  return function (level, colorizer, {customLevels, customLevelNames} = {}) {
    ...
    let levelNumber = 'default';
    if (Number.isInteger(Number(level))) {
      levelNumber = Object.prototype.hasOwnProperty.call(levels, level)
        ? level
        : levelNumber;
    } else {
      levelNumber = Object.prototype.hasOwnProperty.call(
        levelNames,
        level.toLowerCase(),
      )
    ...
    const levelString = levels[levelNumber];

    return Object.prototype.hasOwnProperty.call(colorizer, levelNumber)
      ? colorizer[levelNumber](levelString)
      : colorizer.default(levelString);
  };
}

function plainColorizer(useOnlyCustomProps) {
  const newPlainColorizer = colorizeLevel(useOnlyCustomProps);
  const customColoredColorizer = function (level, options) {
    return newPlainColorizer(level, plain, options);
  };
...
function coloredColorizer(useOnlyCustomProps) {
  ...
  const customColoredColorizer = function (level, options) {
    return newColoredColorizer(level, colored, options);
  };
...
function customColoredColorizerFactory(customColors, useOnlyCustomProps) {
  ...
  const customColoredColorizer = function (level, options) {
    return colorizeLevelCustom(level, customColored, options);
  };
